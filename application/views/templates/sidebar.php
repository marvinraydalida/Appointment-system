...
<head>
    ...
    <?php elseif (strcasecmp($_SERVER['REQUEST_URI'], $request_uri[0]) == 0) : ?>
        <?php
        if (strcasecmp($_SERVER['REQUEST_URI'], $request_uri[0]) == 0) {
            $route = explode("/", $request_uri[0]);
            $route = explode("?", $route[3]);
        }
        ?>
        <?php if (strcasecmp($route[0], "Logs") == 0) : ?>
            <link href=<?php echo site_url("assets/css/adminLogs.css") ?> rel="stylesheet">
        <?php elseif (strcasecmp($route[0], "Appointment") == 0) : ?>
...
<body>
    <section id="sidebar">
        <div id="link-container">
            ...
            <a href="<?php echo site_url('Admin/Logs') . '?dateLog=' . $date . '&action=All' ?>"><i class="fa-solid fa-file-lines"></i> Logs</a>
